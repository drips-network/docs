...
2. **Have a Stellar Wallet**: You'll need a Stellar wallet that supports USDC. Popular options include [Lobstr](https://lobstr.co/) (mobile app), [Ledger](https://www.ledger.com/) (hardware wallet), or any other wallet compatible with the Stellar network, and capable of receiving assets such as USDC. Some centralized exchanges may also support direct USDC deposits on Stellar, but be sure to include a memo if required by the exchange. We recommend always requesting a test transaction to confirm wallet compatibility before withdrawing your full grant amount.
3. **Activate the wallet and enable the USDC Trustline**: Stellar wallets require a minimum balance to be active, and you must enable the USDC *trustline* to be able to receive USDC. Follow your wallet provider's instructions to fund your wallet and add the USDC asset. The instructions for enabling USDC on the popular LOBSTR wallet can be found [here](https://lobstr.freshdesk.com/support/solutions/articles/151000001061-adding-custom-assets-in-lobstr).
     - If you are withdrawing to a centralized exchange that supports direct USDC deposits on Stellar, you don't need to set up the trustline yourself. However, you must ensure that you include the correct memo with your withdrawal request, as the exchange uses the memo to credit your account.

:::danger
**Important**: If your withdrawal address does not have the USDC trustline enabled, your transaction may fail. Always verify your wallet is properly configured before requesting a full withdrawal. **We recommend always using the test transaction feature first to confirm everything is set up correctly.**
:::
...
:::warning
**Memo Required?** If you're withdrawing to a centralized exchange that supports USDC deposits on Stellar (popular choices include Coinbase and Binance), you will likely need to include a memo. Check your exchange's deposit instructions carefully. Sending funds without the required memo may result in lost funds. The memo is distinct from your wallet address.
:::
...
- Verify your wallet address is correct and you provided the correct memo, if required.
...
If the exchange supports direct USDC deposits on the Stellar network, yes. However, if provided, you must always include the correct memo with your withdrawal request. Exchanges use memos to credit incoming transfers to the right account. Always check your exchange's deposit instructions carefully. **We recommend requesting a test transaction first to ensure the address and memo are correct.**

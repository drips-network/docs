...
Usually, you do not need to explicitly reject applications. When you assign an applicants, all other pending applications for that issue are automatically marked as "inactive", resulting in them no longer counting toward the contributor's application limits.
